...
class FormSettingController {
	...
	const SETTINGS_KEY = 'request_a_quote_form_setting';
	...
	public function get_form() {
		...
		$conf = get_option( self::SETTINGS_KEY, FormSettings::get_default_form_data() );
	...
	public static  function elex_raq_add_field() {
		...
		if ( 'radio' === $data['type'] || 'checkbox' === $data['type'] ) {
			...
			if ( ! isset( $_POST['data']['options'] ) || ! is_array( $_POST['data']['options'] ) || count( $_POST['data']['options'] ) <= 1 ) {
				wp_send_json_error(
					array(
	...
	public static function elex_raq_save_form_settings_data() {
		...
		$new_settings['title'] = isset( $_POST['title'] ) ? sanitize_text_field( $_POST['title'] ) : 'Quote Request Form';
		...
		$new_settings['success_message'] = isset( $_POST['success_message'] ) ? sanitize_text_field( $_POST['success_message'] ) : 'Your request has been submitted.';
	...
	public function save_form( FormSettings $form ) {
		update_option( self::SETTINGS_KEY, $form->to_array() );
	}
	...
	public static function load_formsettings() {
		...
		include VIEW_PATH . 'listpage_form_setting/form_settings.php';
	}
