...
<?php
		...
		if ( ! empty( $value ) && is_array( $value ) ) {
			if ( isset( $formdata['billing_first_name'] ) ) {
				if ( ! is_string( $formdata['billing_first_name'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'First name should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			} 
			if ( isset( $formdata['billing_last_name'] ) ) {
				
				if ( ! is_string( $formdata['billing_last_name'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Last name should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_company'] ) ) {
				if ( ! is_string( $formdata['billing_company'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Company name should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_email'] ) ) {
				if ( ! filter_var( $formdata['billing_email'], FILTER_VALIDATE_EMAIL ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Email should be in valid format.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_phone'] ) ) {
				if ( ! preg_match( '/^[0-9]{10}+$/', $formdata['billing_phone'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Email should be a int of length 10.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_address_1'] ) ) {
				if ( ! is_string( $formdata['billing_address_1'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Address 1  should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_address_2'] ) ) {
				if ( ! is_string( $formdata['billing_address_2'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Address 2  should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_city'] ) ) {
				if ( ! is_string( $formdata['billing_city'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'City name  should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_state'] ) ) {
				if ( ! is_string( $formdata['billing_state'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'state name  should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_postcode'] ) ) {
				if ( ! is_string( $formdata['billing_postcode'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Postcode  should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			if ( isset( $formdata['billing_country'] ) ) {
				
				if ( ! is_string( $formdata['billing_country'] ) ) {
					return new WP_Error( 'rest_bad_request', esc_html__( 'Country name should be a string.', 'elex_request_a_quote' ), array( 'status' => 400 ) );
				}
			}
			return true;
		}
		return new WP_Error( 'rest_bad_request', esc_html__( 'Address array is empty', 'elex_request_a_quote' ), array( 'status' => 400 ) );
	...
	public static function elex_raq_array_empty( $value, $request, $key ) {
		...
		if ( 'custom_fields' == $key ) {
			return new WP_Error( 'rest_bad_request', esc_html__( 'Custom Fields array is empty', 'elex_request_a_quote' ), array( 'status' => 400 ) );
		}
		return true;
	}
	public static function elex_raq_user_exists( $value, $request, $key ) {
		...
		return new WP_Error( 'rest_bad_request', esc_html__( 'User not found', 'elex_request_a_quote' ), array( 'status' => 400 ) );
	}
	public static function elex_raq_validate_token( $request ) {
		...
		return new WP_Error( 'rest_forbidden', esc_html__( 'Unauthorize Access', 'elex_request_a_quote' ), array( 'status' => 401 ) );
	}
